...
const errorHandler = (key: string) => new Error(`${key} is not set as the environment variable`);

const defaultStringVariable = (key: string, defaultValue: string): Record<string, Joi.StringSchema> => ({
  [key]: Joi.string().default(defaultValue).error(errorHandler(key)),
});

const requiredStringVariable = (key: string): Record<string, Joi.StringSchema> => ({
...
export const configValidation = Joi.object({
  ...
  ...requiredStringVariable(process.env.NODE_ENV === 'production' ? 'REDIS_TLS_URL' : 'REDIS_URL'),
